 ...
 void RUN_MNEMOS_SAVE_request_post ( struct DOMAIN *domain, gchar *path, gchar *agent_uuid, SoupServerMessage *msg, JsonNode *request )
  { if (Json_has_member ( request, "mnemos_BI" ))
    ...
    if (Json_has_member ( request, "mnemos_MONO" ))
     { Json_foreach_array_element ( request, "mnemos_MONO", Mnemo_sauver_un_MONO_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_DI" ))
     { Json_foreach_array_element ( request, "mnemos_DI", Mnemo_sauver_un_DI_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_DO" ))
     { Json_foreach_array_element ( request, "mnemos_DO", Mnemo_sauver_un_DO_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_AI" ))
     { Json_foreach_array_element ( request, "mnemos_AI", Mnemo_sauver_un_AI_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_AO" ))
     { Json_foreach_array_element ( request, "mnemos_AO", Mnemo_sauver_un_AO_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_REGISTRE" ))
     { Json_foreach_array_element ( request, "mnemos_REGISTRE", Mnemo_sauver_un_REGISTRE_by_array, domain ); }
    if (Json_has_member ( request, "mnemos_CI" ))
     { Json_foreach_array_element ( request, "mnemos_CI", Mnemo_sauver_un_CI_by_array, domain ); }
