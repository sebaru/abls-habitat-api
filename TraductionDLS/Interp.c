...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                      dim 05 avr 2009 12:47:37 CEST */
  ...
  { struct DLS_TRAD *Dls_scanner = DlsScanner_get_extra ( scan_instance );
    ...
    if ( Dls_scanner->buffer_used + taille >= Dls_scanner->buffer_size )
     { gint new_taille = Dls_scanner->buffer_used + taille + 1;
       Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_DEBUG, "Buffer too small, trying to expand it to %d)", new_taille );
       gchar *new_Buffer = g_try_realloc( Dls_scanner->Buffer, new_taille );
       if (!new_Buffer)
        { Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_ERR, "Fail to expand buffer. skipping" );
          return;
        }
       Dls_scanner->Buffer      = new_Buffer;
       Dls_scanner->buffer_size = new_taille;
       Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_DEBUG, "Buffer expanded to %d bytes", Dls_scanner->buffer_size );
     }
    Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_DEBUG,
...
/******************************************************************************************************************************/
 void Emettre_erreur_new ( void *scan_instance, gchar *format, ... )
  { static gchar *too_many="Too many events. Limiting output...\n";
    ...
    Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_ERR, "'%s': %s", Json_get_string ( Dls_scanner->PluginNode, "tech_id" ), log );
...
/******************************************************************************************************************************/
 struct ALIAS *New_alias ( void *scan_instance, gchar *tech_id, gchar *acronyme, gint classe, GList *options )
  { gchar chaine[256];
    ...
    Info ( __func__, "dls", "master", LOG_DEBUG, "'%s:%s'", alias->tech_id, alias->acronyme );
...
/******************************************************************************************************************************/
 void New_link( void *scan_instance, gchar *tech_id, gchar *acronyme, GList *options )
  { struct DLS_TRAD *Dls_scanner = DlsScanner_get_extra ( scan_instance );
    ...
    if (!result)
     { Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_ERR, "'%s:%s' not found in DICO", tech_id, acronyme );
...
/******************************************************************************************************************************/
 struct ALIAS *New_external_alias( void *scan_instance, gchar *tech_id, gchar *acronyme, GList *options )
  { struct ALIAS *alias=NULL;
    ...
    JsonNode *result = Rechercher_DICO ( Dls_scanner->domain, tech_id, acronyme );
    if (!result)
     { Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_ERR, "'%s:%s'. Error when searching in DICO", tech_id, acronyme );
       return(NULL);
     }
    ...
    if (alias)                                                                 /* Si trouvé, on considère que le bit est used */
     { alias->used = 1;
       Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_DEBUG, "'%s:%s' found, classe '%s'",
                 alias->tech_id, alias->acronyme, Json_get_string ( result, "classe" ) );
     }
    else { Info ( __func__, "dls", ((Dls_scanner->domain) ? (Dls_scanner->domain)->uuid : "master"), LOG_ERR, "'%s:%s' new_alias not found", tech_id, acronyme ); }
    Json_unref ( result );
    return(alias);
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void Dls_traduire_plugin ( struct DOMAIN *domain, JsonNode *PluginNode )
  { struct ALIAS *alias;
    ...
    if (!PluginNode)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "DLS_TRAD PluginNode Error (is null)" );
       return;
     }
    ...
    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': Starting traduction.", plugin_tech_id );
...
/************************************************ Descend le sourcecode sur disque ********************************************/
    ...
    if (fd_source<0)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': Source creation failed %s (%s)", plugin_tech_id, source, strerror(errno) );
...
/*********************************** Prépare la détection des bits unused en base de données **********************************/
    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': Preparing DB to detect unused bits", plugin_tech_id );
...
/************************************************ Charge un scanner ***********************************************************/
    ...
    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': Copy to disk OK. Parsing in progress", plugin_tech_id );
/*********************************************************** Parsing **********************************************************/
    FILE *rc = fopen( source, "r" );
    if (!rc)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': Open source File Error", plugin_tech_id );
...
/*--------------------------------------- Création des mnemoniques permanents ------------------------------------------------*/
    ...
    if (Dls_scanner->nbr_erreur)
     { Json_add_bool   ( PluginNode, "compil_status", TRUE );                             /* compil ok but errors in dls */
       ...
       Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': %d errors found", plugin_tech_id, Dls_scanner->nbr_erreur );
       End_scanner ( domain, Dls_scanner );
       return;
     }

    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': No parsing error, starting mnemonique import", plugin_tech_id );
...
/*-------------------------------------- Fin de traduction sans erreur + import mnemo ok -------------------------------------*/
    ...
    End_scanner ( domain, Dls_scanner );
    Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "'%s': Compiled in %03.1fs", plugin_tech_id, compil_time/10.0 );
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
