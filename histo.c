...
/******************************************************************************************************************************/
 void HISTO_ACQUIT_request_post ( struct DOMAIN *domain, JsonNode *token, gchar *path, SoupServerMessage *msg, JsonNode *request )
  { if (Http_fail_if_has_not ( domain, path, msg, request, "tech_id")) return;
    ...
    g_free(tech_id);
    g_free(name);
    MQTT_Send_to_domain ( domain, request, "DLS/ACQUIT" );
 ...
 void HISTO_Handle_one ( struct DOMAIN *domain, JsonNode *source )
  { if (!Json_has_member(source, "alive"))    return;
    ...
    if (Json_get_bool ( source, "alive" ) == TRUE)                                                      /* Apparition message */
     { Info ( __func__, "histo", domain->uuid, LOG_DEBUG, "Received MSG '%s:%s' = 1", tech_id, acronyme );
     ...
     { Info ( __func__, "histo", domain->uuid, LOG_DEBUG, "Received MSG '%s:%s' = 0", tech_id, acronyme );
