...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                                30.01.2025 21:05:55 */
  ...
  { gboolean retour = TRUE;
    ...
    if (Json_has_member ( request, "name" ))
     { gchar *name = Normaliser_chaine ( Json_get_string( request, "name" ) );
       ...
       else Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%05d': Normalize Name Error", dls_package_id );
     }
    ...
    if (Json_has_member ( request, "description" ))
     { gchar *description = Normaliser_chaine ( Json_get_string( request, "description" ) );
       ...
       else Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%05d': Normalize Description Error", dls_package_id );
     }
    ...
    if (Json_has_member ( request, "sourcecode" ))
     { gchar *sourcecode = Normaliser_chaine ( Json_get_string( request, "sourcecode" ) );
       ...
       else Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%05d': Normalize Sourcecode Error", dls_package_id );
     }
...
/******************************************************************************************************************************/
 gboolean Dls_Apply_package ( struct DOMAIN *domain, JsonNode *PluginNode )
  { if (!Json_has_member ( PluginNode, "tech_id" )) return(FALSE);
    ...
    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': Searching for a package", tech_id );
    ...
    if (!retour)
    { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': DB read error", tech_id );
      return(FALSE);
    }

    if (!Json_has_member( PluginNode, "package" ))
    { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': tech_id not not found", tech_id );
      return(FALSE);
    }
    ...
    if ( ! (package && strlen(package) && strcasecmp ( package, "custom" ) ) )                     /* Ce n'est pas un package */
     { if (Json_has_member ( PluginNode, "sourcecode" ) )
        { Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': is not a package. Using embedded sourcecode from jsonnode", tech_id );
          return(TRUE);
        }
       Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': is not a package. Using sourcecode from database", tech_id );
       DB_Read ( domain, PluginNode, NULL, "SELECT sourcecode FROM dls WHERE tech_id='%s'", tech_id );
       if (!Json_has_member( PluginNode, "sourcecode" ))
        { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': sourcode not not found in database.", tech_id );
          return(FALSE);
        }
       return(TRUE);
     }

    Info ( __func__, "dls", domain->uuid, LOG_INFO, "'%s': Applying package '%s'", tech_id, package );
...
/**************************************** Essaie avec un package local s'il existe ********************************************/
    ...
    g_free(name);
    if (!retour)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': DB Sourcecode error", tech_id );
       return(FALSE);
     }
...
/************************************ Sinon on essaie de le télécharger depuis le site static *********************************/
    ...
    if (error)
     { gchar *uri = g_uri_to_string(soup_message_get_uri(soup_msg));
       Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': Unable to retrieve Package '%s': error %s", tech_id, package_query, error->message );
       g_free(uri);
       g_error_free ( error );
     }
