...
MOD_FOUND=$(echo "${RESPONSE}" | jq -r '.modbus[] | select(.thread_tech_id == "TEST_MODBUS") | .thread_tech_id' 2>/dev/null)
_test_start
...
RESPONSE=$(api_call POST /modbus/set "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","description":"Automate de test modifié","hostname":"192.168.1.200","watchdog":50,"max_request_par_sec":50}')

assert_http_status 200 "POST /modbus/set → HTTP 200"

DESC_DB=$(db_domain_query "SELECT description FROM modbus WHERE thread_tech_id='TEST_MODBUS' LIMIT 1;")
_test_start
...
api_call POST /modbus/set "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","description":"Automate de test","hostname":"192.168.1.200","watchdog":50,"max_request_par_sec":50}' >/dev/null
...
RESPONSE=$(api_call POST /modbus/set "${READONLY_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","description":"Tentative","hostname":"x","watchdog":50,"max_request_par_sec":50}')
...
RESPONSE=$(api_call POST /modbus/set/di "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","thread_acronyme":"MOD_DI_01","libelle":"DI Test modifiée","num":0,"borne":"I1","ed":"DI","flip":false,"archivage":36000}')

assert_http_status 200 "POST /modbus/set/di → HTTP 200"

DI_LIBELLE=$(db_domain_query "SELECT libelle FROM modbus_DI WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_DI_01' LIMIT 1;")
_test_start
...
# Restaurer
db_domain_query "UPDATE modbus_DI SET libelle='Entrée digitale test 01' WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_DI_01';" >/dev/null 2>&1 || true
...
RESPONSE=$(api_call POST /modbus/set/do "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","thread_acronyme":"MOD_DO_01","libelle":"DO Test modifiée","num":0,"borne":"Q1","ed":"DO","archivage":36000}')

assert_http_status 200 "POST /modbus/set/do → HTTP 200"

DO_LIBELLE=$(db_domain_query "SELECT libelle FROM modbus_DO WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_DO_01' LIMIT 1;")
_test_start
...
db_domain_query "UPDATE modbus_DO SET libelle='Sortie digitale test 01' WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_DO_01';" >/dev/null 2>&1 || true
...
RESPONSE=$(api_call POST /modbus/set/ai "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","thread_acronyme":"MOD_AI_01","libelle":"AI Test modifiée","num":0,"type_borne":0,"min":0,"max":100,"borne":"IW1","ed":"AI","unite":"%","archivage":36000}')

assert_http_status 200 "POST /modbus/set/ai → HTTP 200"

AI_LIBELLE=$(db_domain_query "SELECT libelle FROM modbus_AI WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_AI_01' LIMIT 1;")
_test_start
...
db_domain_query "UPDATE modbus_AI SET libelle='Entrée analogique test 01' WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_AI_01';" >/dev/null 2>&1 || true
...
RESPONSE=$(api_call POST /modbus/set/ao "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    '{"thread_tech_id":"TEST_MODBUS","thread_acronyme":"MOD_AO_01","libelle":"AO Test modifiée","num":0,"type_borne":0,"min":0,"max":100,"borne":"QW1","ed":"AO","unite":"%","archivage":36000}')

assert_http_status 200 "POST /modbus/set/ao → HTTP 200"

AO_LIBELLE=$(db_domain_query "SELECT libelle FROM modbus_AO WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_AO_01' LIMIT 1;")
_test_start
...
db_domain_query "UPDATE modbus_AO SET libelle='Sortie analogique test 01' WHERE thread_tech_id='TEST_MODBUS' AND thread_acronyme='MOD_AO_01';" >/dev/null 2>&1 || true
