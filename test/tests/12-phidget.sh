...
# =============================================================================
# Endpoints testés: GET /phidget/list?classe=IO, POST /phidget/set, POST /phidget/set/io
...
log_info "Test: GET /phidget/list?classe=IO"
RESPONSE=$(api_call GET "/phidget/list?classe=IO" "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}")

assert_http_status 200 "GET /phidget/list?classe=IO → HTTP 200"

PHI_FOUND=$(echo "${RESPONSE}" | jq -r '.IO[] | select(.agent_tech_id == "TEST_PHIDGET") | .agent_tech_id' 2>/dev/null)
_test_start
if [[ "${PHI_FOUND}" == "TEST_PHIDGET" ]]; then
    _test_pass "GET /phidget/list?classe=IO contient TEST_PHIDGET"
else
    _test_fail "GET /phidget/list?classe=IO ne contient pas TEST_PHIDGET" "${RESPONSE}"
fi

PHIDGET_DB=$(db_domain_query "SELECT COUNT(*) FROM phidget_IO;")
PHIDGET_API=$(echo "${RESPONSE}" | jq '.IO | length' 2>/dev/null)
_test_start
if [[ "${PHIDGET_API}" == "${PHIDGET_DB}" ]]; then
    _test_pass "GET /phidget/list?classe=IO nombre cohérent avec BD (${PHIDGET_DB})"
else
    _test_fail "GET /phidget/list?classe=IO nombre incohérent" "API=${PHIDGET_API}, BD=${PHIDGET_DB}"
fi

log_info "Test: GET /phidget/list?classe=IO - readonly (accès insuffisant)"
RESPONSE=$(api_call GET "/phidget/list?classe=IO" "${READONLY_TOKEN}" "${TEST_DOMAIN_UUID}")
assert_http_status 403 "GET /phidget/list?classe=IO readonly → HTTP 403"
...
RESPONSE=$(api_call POST /phidget/set/io "${ADMIN_TOKEN}" "${TEST_DOMAIN_UUID}" \
    "{\"phidget_io_id\":${PHI_IO_ID},\"capteur\":\"DIGITAL-INPUT\",\"libelle\":\"IO Phidget modifié\",\"intervalle\":5000,\"archivage\":36000}")
...
db_domain_query "UPDATE phidget_IO SET classe='DI', port=0, capteur='', libelle='Entrée phidget test 01', intervalle=5000, archivage=36000 WHERE agent_tech_id='TEST_PHIDGET' AND thread_acronyme='PHI_IO_01';" >/dev/null 2>&1 || true
