...
/******************************************************************************************************************************/
 static void Dls_commit_plugin ( struct DOMAIN *domain, JsonNode *PluginNode )
  { gchar *tech_id = Json_get_string ( PluginNode, "tech_id" );
    ...
    if (fd==-1)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': mkstemp failed", tech_id );
       return;
     }

    gchar *sourcecode = Json_get_string ( PluginNode, "sourcecode" );
    if (write ( fd, sourcecode, strlen(sourcecode) ) < 0)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': writing file failed", tech_id );
       close(fd);
       goto end;
     }
    close(fd);
    ...
    if (pid == 0) // Code exécuté par le processus fils
     { gchar filepath[256];
       ...
       Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': execl Error", tech_id );
       return;
     }

    int status;
    waitpid(pid, &status, 0); // Attendre que le processus fils se termine
    if (WIFEXITED(status)) Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "'%s': plugin committed", tech_id );
                      else Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': commit plugin error", tech_id );
end:
 ...
 void Dls_Compil_one ( struct DOMAIN *domain, JsonNode *token, JsonNode *plugin )
  { gchar *tech_id = Json_get_string ( plugin, "tech_id" );
    ...
    if (Dls_Apply_package ( domain, plugin ) == FALSE)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': Error when applying package / sourcode. Dropping.", tech_id ); return; }
    ...
    if (Json_get_bool ( plugin, "compil_status" ) && Json_get_int ( plugin, "error_count" ) == 0 )
     { Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "'%s': Parsing OK, sending Compil Order to Master Agent", tech_id );
       ...
       if (info)
        { info->domain     = domain;
          ...
          if ( pthread_create( &TID, &attr, (void *)DLS_Compil_one_by_thread, info ) )
           { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': pthread_create failed.", tech_id );
  ...
  { JsonNode *pluginsNode = Json_create();
    ...
    if (retour)
     { gint nbr_plugin = Json_get_int ( pluginsNode, "nbr_plugins" );
       Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "Start compiling %03d plugins (with pattern'%s').", nbr_plugin, pattern );
       ...
       Info ( __func__, "dls", domain->uuid, LOG_INFO, "Compil %03d plugins in %06.1fs", nbr_plugin, (Global.Top - compil_top)/10.0 );
     } else Info ( __func__, "dls", domain->uuid, LOG_ERR, "Database Error searching for plugins. Compil aborted." );
 ...
 void DLS_LIST_request_get ( struct DOMAIN *domain, JsonNode *token, const char *path, SoupServerMessage *msg, JsonNode *url_param )
  {
    ...
    retour &= DB_Read ( domain, RootNode, NULL,
              "SELECT agent_tech_id AS master_hostname FROM servers WHERE is_master=1" );
 ...
 void RUN_DLS_CREATE_request_post ( struct DOMAIN *domain, gchar *path, gchar *agent_uuid, SoupServerMessage *msg, JsonNode *request )
  { if (Http_fail_if_has_not ( domain, path, msg, request, "tech_id" ))     return;
    ...
    if (!tech_id_safe || !shortname_safe || !name_safe || !package_safe)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "'%s': Normalize Error", (tech_id ? tech_id : "unknown") );
       Http_Send_json_response ( msg, SOUP_STATUS_INTERNAL_SERVER_ERROR, "Normalize Error", NULL );
       goto end;
     }
    ...
    if(retour)
     { Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "D.L.S plugin '%s' created ('%s' - '%s')", tech_id, shortname, name );
...
/******************************************************************************************************************************/
 static void DLS_COMPIL_ALL_CB ( struct HTTP_COMPIL_REQUEST *http_request )
  { struct DOMAIN *domain = http_request->domain;
    ...
    if (!retour)
     { Info ( __func__, "dls", domain->uuid, LOG_ERR, "Database Error searching for plugins. Compil_all aborted." );
       goto end;
     }

    gint nbr_plugin = Json_get_int ( pluginsNode, "nbr_plugins" );
    Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "Start compiling %03d plugins (with dls_package_id=%d).", nbr_plugin, dls_package_id );
    gint compil_top = Global.Top;
    DLS_Compil_with_array ( domain, token, Json_get_array ( pluginsNode, "plugins" ) );
    while(Global.Nbr_compil) sched_yield();                                             /* Attente de toutes les compilations */
...
/************************************************** S'agit-il d'un package ? **************************************************/
    ...
    Info ( __func__, "dls", domain->uuid, LOG_NOTICE, "'%s': Parsing OK (in %06.1fs), sending Compil Order to Master Agent", tech_id, compil_time/10.0 );
