...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                                09.04.2022 21:33:35 */
 ...
 void USER_PROFIL_request_get ( JsonNode *token, SoupServerMessage *msg )
  {
    ...
    if (!Json_has_member ( RootNode, "user_uuid" ))
     { Info ( __func__, "user", "master", LOG_NOTICE, "First request of a new user '%s'. Creating entry in database", email );
     ...
     } else Info ( __func__, "user", "master", LOG_NOTICE, "User '%s' ('%s') found.", user_uuid, email );
...
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 void RUN_USERS_WANNA_BE_NOTIFIED_request_get ( struct DOMAIN *domain, gchar *path, struct ABLS_HEADERS *abls_headers, SoupServerMessage *msg, JsonNode *url_param )
  { JsonNode *RootNode = Http_json_node_create (msg);
    if (!RootNode) return;
...
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 void RUN_USER_CAN_SEND_TXT_CDE_request_post ( struct DOMAIN *domain, gchar *path, struct ABLS_HEADERS *abls_headers, SoupServerMessage *msg, JsonNode *request )
  {
    gchar *critere = NULL;
