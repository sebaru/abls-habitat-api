...
/* Projet Abls-Habitat version 4.7   Gestion d'habitat                                                    16.02.2022 09:43:52 */
...
/******************************************************************************************************************************/
 void Info_new( gchar *function, gchar *context, guint priority, struct DOMAIN *domain, gchar *format, ... )
  { gchar chaine[512], nom_thread[32], *domain_uuid;
    ...
    g_snprintf( chaine, sizeof(chaine), "{ \"domain_uuid\": \"%s\", \"context\":\"%s\", \"thread\":\"%s\", \"function\":\"%s\", \"message\":\"%s\" }",
                domain_uuid, (context ? context : ""), nom_thread, function, format );
...
/******************************************************************************************************************************/
 void Info_change_log_level( guint new_log_level )
  { setlogmask( LOG_UPTO(new_log_level) );
    Info_new( __func__, "log", LOG_INFO, NULL, "Log level set to %d", new_log_level );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static void Info_stop( int code_retour, void *data )
  { Info_new( __func__, "log", LOG_INFO, NULL, "End of logs" ); }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void Info_init( gchar *entete, guint log_level )
  { on_exit( Info_stop, NULL );
    ...
    Info_new( __func__, "log", LOG_INFO, NULL, "Start of logs" );
