...
/*--------------------------------------------- Requetes GET des users (dans un domaine) -------------------------------------*/
    if (soup_server_message_get_method ( msg ) == SOUP_METHOD_GET)
     {      if (!strcasecmp ( path, "/histo/alive" ))      HISTO_ALIVE_request_get     ( domain, token, path, msg, url_param );
       ...
       else if (!strcasecmp ( path, "/modbus/get" ))       MODBUS_GET_request_get      ( domain, token, path, msg, url_param );
...
/*--------------------------------------------- Requetes POST des users (dans un domaine) ------------------------------------*/
     ...
     { request = Http_Msg_to_Json ( msg );
       ...
       else if (!strcasecmp ( path, "/server/set/headless" )) SERVER_SET_HEADLESS_request_post ( domain, token, path, msg, request );
