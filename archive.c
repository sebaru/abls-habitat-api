 ...
 void ARCHIVE_REBUILD_request_post ( struct DOMAIN *domain, JsonNode *token, const char *path, SoupServerMessage *msg, JsonNode *request )
  { if (!Http_is_authorized ( domain, token, path, msg, 6 )) return;
    ...
    if ( pthread_create( &TID, NULL, (void *)ARCHIVE_REBUILD_thread, thread_info ) )
     { Info ( __func__, "archive", domain->uuid, LOG_ERR, "Error while pthreading DB_Cleanup: %s", strerror(errno) );
...
/******************************************************************************************************************************/
 static void ARCHIVE_Move_hot_to_cold ( struct DOMAIN *domain )
  { gchar src_partname[16], dst_tablename[16];
    ...
    if (!cold_retention) { Info ( __func__, "archive", domain->uuid, LOG_ERR, "Cold Retention is disabled." ); return; }
    ...
    Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Move '%d' partitions to cold table", Json_get_int ( RootNode, "nbr_partitions" ) );
    ...
    while(parts)
     { JsonNode *element = parts->data;
       ...
       Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Create cold table '%s'", dst_tablename );
       DB_Arch_Write ( domain, "CREATE TABLE IF NOT EXISTS `%s` LIKE `histo_bit`", dst_tablename );

       Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Hot to Cold: move partition '%s' to table '%s'", src_partname, dst_tablename );
       if ( DB_Arch_Write ( domain, "INSERT INTO `%s` (tech_id, acronyme, date_time, valeur) "
        ...
        { Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Moving OK: Now delete old partition '%s'", src_partname );
          DB_Arch_Write ( domain, "ALTER TABLE `histo_bit` DROP PARTITION %s;", src_partname );
        } else Info ( __func__, "archive", domain->uuid, LOG_ERR, "Error when moving hot '%s' to cold '%s'", src_partname, dst_tablename );
 ...
 void ARCHIVE_HOT_TO_COLD_request_post ( struct DOMAIN *domain, JsonNode *token, const char *path, SoupServerMessage *msg, JsonNode *request )
  { if (!Http_is_authorized ( domain, token, path, msg, 6 )) return;
    ...
    if ( pthread_create( &TID, NULL, (void *)ARCHIVE_Move_hot_to_cold, domain ) )
     { Info ( __func__, "archive", domain->uuid, LOG_ERR, "Error while pthreading: %s", strerror(errno) ); }
    else
...
/******************************************************************************************************************************/
 static void ARCHIVE_Delete_old_cold ( struct DOMAIN *domain )
  { struct tm prev;
    ...
    Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Delete '%d' old cold tables < '%d'",
    ...
    while(tables)
     { JsonNode *element = tables->data;
       ...
       Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Delete old cold table '%s'", tablename );
...
/*------------------------------------- Création de la nouvelle partition du mois --------------------------------------------*/
    if (Global.Top_localtime.tm_mday == 1)                                                         /* Si premier jour du mois */
     { struct tm prev;
       ...
       Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "First Day of month, create partition 'p_%d%02d'", prev.tm_year+1900, prev.tm_mon+1 );
       DB_Write ( domain, "INSERT INTO cleanup SET archive = 1, "
    ...
    if (!RootNode)
     ...
     { DB_Arch_Read ( domain, 0, RootNode, NULL,
       ...
       if (pct_unused > 5.0)                                                     /* Pour toute table fragmentée de plus de 5% */
        { Info ( __func__, "archive", domain->uuid, LOG_NOTICE, "Rebuilding partition '%s' with pct_unused=%f%%", partition, pct_unused );
