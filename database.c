 ...
 gchar *Normaliser_chaine( gchar *pre_comment )
  { gchar *comment, *source, *cible;
    ...
    if (!g_utf8_validate( pre_comment, -1, &end ))                                                      /* Validate la chaine */
     { Info ( __func__, "database", "master", LOG_WARNING, "Could not validate UTF8 string" );
       Info ( __func__, "database", "master", LOG_WARNING, "%s", pre_comment );
       return(NULL);
     }
 ...
 static gint DB_Pool_take ( struct DOMAIN *domain )
  { if (!domain) return(-1);
    if (!domain->db_slot[0].db_mysql)
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "No pool available. Starting." );
       if (!DB_Pool_init ( domain ))
        { Info ( __func__, "database", domain->uuid, LOG_ERR, "Failed to start DB_Pool. Dropping." );
          return(-1);
        }
     }
    ...
    Info ( __func__, "database", domain->uuid, LOG_ERR, "All pool are busy." );
    return(-1);
  }
/******************************************************************************************************************************/
    ...
    if (!domain->arch_db_slot[0].db_mysql)
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "No pool available. Starting." );
       if (!DB_Arch_Pool_init ( domain ))
        { Info ( __func__, "database", domain->uuid, LOG_ERR, "Failed to start DB_Arch_Pool. Dropping." );
          return(-1);
        }
     }
...
/******************************************************************************************************************************/
 gboolean DB_Write( struct DOMAIN *domain, gchar *format, ... )
  { gboolean retour = FALSE;
    ...
    if (i == -1)
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "DB FAILED: No pool available for '%s'", requete );
       retour = FALSE;
     }
    else if ( mysql_query ( domain->db_slot[i].db_mysql, requete ) )
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "DB FAILED: %s for '%s'", (char *)mysql_error(domain->db_slot[i].db_mysql), requete );
       g_snprintf ( domain->mysql_last_error, sizeof(domain->mysql_last_error), "%s", (char *)mysql_error(domain->db_slot[i].db_mysql) );
       retour = FALSE;
     }
    else
     { Info ( __func__, "database", domain->uuid, LOG_DEBUG, "DB OK in %04.1fs: '%s'", (Global.Top - top) / 10.0, requete );
       retour = TRUE;
     }
 ...
 gboolean DB_Arch_Write( struct DOMAIN *domain, gchar *format, ... )
  { gboolean retour = FALSE;
    ...
    if (i==-1)
     ...
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "DB FAILED: %s for '%s'", (char *)mysql_error(domain->arch_db_slot[i].db_mysql), requete );
...
/******************************************************************************************************************************/
 gboolean DB_Pool_init ( struct DOMAIN *domain )
  { if (!domain)
     { Info ( __func__, "database", "master", LOG_ERR, "No domain selected, DBConnect failed" );
       return(FALSE);
     }
    ...
    if (!Json_has_member ( domain->config, "db_password" ))
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "db_password is missing. DBConnect failed." );
       return(FALSE);
     }
    ...
    for (i=0; i<DATABASE_POOL_SIZE; i++)
     { domain->db_slot[i].db_mysql = mysql_init(NULL);
       if (!domain->db_slot[i].db_mysql)
        { Info ( __func__, "database", domain->uuid, LOG_ERR, "Unable to init domain database pool %d", i );
          break;
        }
       ...
       if ( ! mysql_real_connect( domain->db_slot[i].db_mysql, db_hostname, db_username, db_password, db_database, db_port, NULL, 0 ) )
        { Info ( __func__, "database", domain->uuid, LOG_ERR, "Mysql_real_connect failed to connect to %s@%s:%d/%s for pool %d -> %s",
        ...
        { domain->db_slot[i].db_cache = memcached ( memcached_options, memcached_options_size );
          if (domain->db_slot[i].db_cache)
             { Info ( __func__, "database", domain->uuid, LOG_NOTICE, "Slot '%d': Using DB Cache with options '%s'", i, memcached_options ); }
          else Info ( __func__, "database", domain->uuid, LOG_WARNING, "slot '%d': Error, not using DB Cache (options '%s')", i, memcached_options );
        }
     }

    if (!i)
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "Cannot load any DBPool for %s@%s:%d on %s", db_username, db_hostname, db_port, db_database );
       return(FALSE);
     }
...
/******************************************************************************************************************************/
 gboolean DB_Arch_Pool_init ( struct DOMAIN *domain )
  { if (!domain)
    ...
    if (!Json_has_member ( domain->config, "db_password" ))
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "db_password is missing. DBArchConnect failed." );
       return(FALSE);
     }
    ...
    if (!i)
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "Cannot load any DBArchPool for %s@%s:%d on %s", db_username, db_hostname, db_port, db_database );
       return(FALSE);
     }

    Info ( __func__, "database", domain->uuid, LOG_INFO, "%d Pools OK with %s@%s:%d on %s", i, db_username, db_hostname, db_port, db_database );
    return(TRUE);
  }
  ...
  { struct DOMAIN *master = DOMAIN_tree_get ( "master" );
    ...
    if ( categorie && forme && extension && controle )
     { DB_Write ( master,
       ...
       Info ( __func__, "database", "master", LOG_INFO, "Icon '%s:%s' control '%s' imported", categorie, forme, controle );
     }
    else Info ( __func__, "database", "master", LOG_ERR, "Error when importing icon '%s'", Json_get_string ( element, "forme" ) );
  ...
  { gboolean retour = FALSE;
    ...
    if (error)
     { gchar *uri = g_uri_to_string(soup_message_get_uri(soup_msg));
       Info ( __func__, "database", "master", LOG_ERR, "Unable to retrieve ICON INVENTORY on %s: error %s", icon_query, error->message );
       g_free(uri);
       g_error_free ( error );
     }
     ...
     { gsize taille;
       ...
       if (taille && buffer_safe)
        { memcpy ( buffer_safe, buffer_unsafe, taille );                                        /* Copy with \0 end of string */
          ...
          if (!ResponseNode)
           { Info ( __func__, "database", "master", LOG_ERR, "Unable to retrieve ICON INVENTORY on %s: 'inventory.json' not json", icon_query ); }
          else if (!Json_has_member ( ResponseNode, "icons" ))
           { Info ( __func__, "database", "master", LOG_ERR, "Unable to retrieve ICON INVENTORY on %s: 'inventory.json' do not have 'icons' array", icon_query ); }
          else
           { Json_foreach_array_element ( ResponseNode, "icons", DB_Load_one_icon, NULL );
             Info ( __func__, "database", "master", LOG_NOTICE, "ICON INVENTORY loaded from %s", icon_query );
             retour = TRUE;
           }
          g_free(buffer_safe);
    ...
    else Info ( __func__, "database", "master", LOG_CRIT, "Unable to retrieve ICON INVENTORY on %s: error %s", icon_query, reason_phrase );
...
/******************************************************************************************************************************/
 gboolean DB_Master_Update ( void )
  { struct DOMAIN *master = DOMAIN_tree_get ( "master" );
    ...
    Info ( __func__, "database", "master", LOG_INFO, "Master Schema Updated to version '%d'", version );
    return(TRUE);
  }
/******************************************************************************************************************************/
...
/*--------------------------------- si pas de cache ou cache failed, on tente via SGBD ---------------------------------------*/
    if ( mysql_query ( mysql, requete ) )                                           /* Envoi de la requete au serveur de SGBD */
     { Info ( __func__, "database", domain->uuid, LOG_ERR, "DB FAILED (%s) for '%s'", (char *)mysql_error(mysql), requete );
    ...
    if ( ! result )
     { Info ( __func__, "database", domain->uuid, LOG_WARNING, "Store_result failed (%s)", (char *) mysql_error(mysql) );
       g_snprintf ( domain->mysql_last_error, sizeof(domain->mysql_last_error), "%s", (char *)mysql_error(mysql) );
       retour = FALSE; goto end;
     }
...
/*------------------------------------------------ Mise en cache -------------------------------------------------------------*/
   if (cache_retention && domain->db_slot[i].db_cache)
    ...
    gettimeofday(&time_end, NULL);
    Info ( __func__, "database", domain->uuid, LOG_DEBUG, "DB OK in %.3fms: '%s'",
              (time_end.tv_sec - time_start.tv_sec) * 1000.0 + (time_end.tv_usec - time_start.tv_usec) / 1000.0,
              requete );
    retour = TRUE;
end:
...
/******************************************************************************************************************************/
 static void DB_Cleanup_thread ( struct DOMAIN *domain )
  { prctl(PR_SET_NAME, "W-CleanSQL", 0, 0, 0 );
    Info ( __func__, "database", domain->uuid, LOG_NOTICE, "Starting DB_Cleanup_thread" );
  ...
  { struct DOMAIN *domain = value;
    ...
    if(domain->database_cleanup_TID == 0)
     { if ( pthread_create( &domain->database_cleanup_TID, NULL, (void *)DB_Cleanup_thread, domain ) )
       { Info ( __func__, "database", domain->uuid, LOG_ERR, "Error while pthreading DB_Cleanup: %s", strerror(errno) ); }
     }
