...
/******************************************** Préparation de la base du mnemo *************************************************/
    ...
    if ( !acro )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for acronyme." ); }

    libelle    = Normaliser_chaine ( libelle_src );                                          /* Formatage correct des chaines */
    if ( !libelle )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for libelle." ); }

    forme      = Normaliser_chaine ( forme_src );                                            /* Formatage correct des chaines */
    if ( !forme )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for forme." ); }

    mode       = Normaliser_chaine ( mode_src );                                             /* Formatage correct des chaines */
    if ( !mode )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for mode." ); }

    couleur    = Normaliser_chaine ( couleur_src );                                          /* Formatage correct des chaines */
    if ( !couleur )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for couleur." ); }

    input_tech_id  = Normaliser_chaine ( input_tech_id_src );                                /* Formatage correct des chaines */
    if ( !input_tech_id )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for input_tech_id." ); }

    input_acronyme = Normaliser_chaine ( input_acronyme_src );                               /* Formatage correct des chaines */
    if ( !input_acronyme )
     { Info ( __func__, "visuel", domain->uuid, LOG_ERR, "Normalize error for input_acronyme." ); }
