...
/******************************************************************************************************************************/
 gboolean Send_mail ( gchar *sujet, gchar *dest, gchar *body )
  { gchar fichier[32], commande[128], chaine[256];
    ...
    fd = mkstemp ( fichier );
    if (fd==-1)
     { Info ( __func__, "mail", "master", LOG_ERR, "Mkstemp failed for '%s', '%s': %s", sujet, dest, strerror(errno) );
       return(FALSE);
     }
    ...
    if (write ( fd, chaine, strlen(chaine) ) < 0)
     { Info ( __func__, "mail", "master", LOG_ERR, "%s: writing smtp header failed for '%s', '%s'", sujet, dest );
       close(fd);
       return(FALSE);
     }

    if (write ( fd, mail_header, strlen(mail_header) ) < 0)
     { Info ( __func__, "mail", "master", LOG_ERR, "%s: writing mail header failed for '%s', '%s'", sujet, dest );
       close(fd);
       return(FALSE);
     }

    if (write ( fd, body, strlen(body) ) < 0)
     { Info ( __func__, "mail", "master", LOG_ERR, "Writing body failed for '%s', '%s'", sujet, dest );
       close(fd);
       return(FALSE);
     }

    if (write ( fd, mail_footer, strlen(mail_footer) ) < 0)
     { Info ( __func__, "mail", "master", LOG_ERR, "%s: writing mail footer failed for '%s', '%s'", sujet, dest );
       close(fd);
       return(FALSE);
     }
    ...
    system(commande);
    unlink(fichier);
    Info ( __func__, "mail", "master", LOG_NOTICE, "Mail '%s' sent to '%s'", sujet, dest );
    return(TRUE);
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
