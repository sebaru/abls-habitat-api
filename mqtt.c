...
/* Projet Abls-Habitat version 4.7   Gestion d'habitat                                                    12.07.2024 18:00:51 */
...
/******************************************************************************************************************************/
 static void MQTT_API_on_log_CB( struct mosquitto *mosq, void *obj, int level, const char *message )
  { gint info_level;
    ...
    Info ( __func__, "mqtt", "master", info_level, "%s", message );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static void MQTT_API_on_message_CB ( struct mosquitto *MQTT_session, void *obj, const struct mosquitto_message *msg )
  { gchar **tokens = g_strsplit ( msg->topic, "/", 5 );
    ...
    if (!domain)
     { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT Message from unknown domain. Dropping for topic %s !", msg->topic );
       goto end;
     }

    if (!msg->payload)
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Message with no payload on topic %s", msg->topic );
       goto end;
     }

    JsonNode *request = Json_get_from_string ( msg->payload );
    if (!request)
     { Info ( __func__, "mqtt", domain->uuid, LOG_WARNING, "MQTT Message Dropped (not JSON) for topic %s !", msg->topic );
       goto end;
     }

    Info ( __func__, "mqtt", domain->uuid, LOG_DEBUG, "Received %s: %s", tokens[1], msg->payload );
 ...
 static void Mqtt_Allow_pattern_for ( struct DOMAIN *domain, gchar *suffixe, gchar *topic )
  { gchar commande[1024];
    ...
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Add %s-%s subscription for %s/%s failed, error %s",
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Add %s-%s publishClientSend for %s/%s failed, error %s",
                 domain_uuid, suffixe, domain_uuid, topic, mosquitto_strerror(retour) );
     }
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Add %s-%s publishClientReceive for %s/%s failed, error %s",
                 domain_uuid, suffixe, domain_uuid, topic, mosquitto_strerror(retour) );
     }
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void MQTT_Allow_one_domain ( struct DOMAIN *domain )
  { gchar commande[1024];
    gint retour;
    if (!domain) { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT Allow one domain failed, no domain provided" ); return; }
...
/*------------------------------------------------------- Create Role --------------------------------------------------------*/
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Create Agent Role failed, error %s", mosquitto_strerror(retour) ); }
...
/*------------------------------------------------------- Create Client ------------------------------------------------------*/
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Create Agents domain failed, error %s", mosquitto_strerror(retour) ); }
/*------------------------------------------------------- Create Browser Role ------------------------------------------------*/
    g_snprintf ( commande, sizeof(commande),
                 "{ \"commands\":[ "
                 "  { "
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Create Browsers Role failed, error %s", mosquitto_strerror(retour) ); }
...
/*------------------------------------------------------- Create Browsers  ------------------------------------------------------*/
    ...
    retour = mosquitto_publish( Global.MQTT_session, NULL, "$CONTROL/dynamic-security/v1", strlen(commande), commande, 2, FALSE );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", domain->uuid, LOG_ERR, "MQTT Create Browsers domain failed, error %s", mosquitto_strerror(retour) ); }
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static void MQTT_API_on_connect_CB( struct mosquitto *mosq, void *obj, int return_code )
  { Info ( __func__, "mqtt", "master", LOG_NOTICE, "Connected with return code %d: %s",
    ...
    if (return_code == 0)
     { gboolean retour = mosquitto_subscribe( Global.MQTT_session, NULL, "+/DLS_VISUEL/#", 1 );
       if ( retour != MOSQ_ERR_SUCCESS )
        { Info ( __func__, "mqtt", "master", LOG_ERR, "Subscribe to topic 'DLS_VISUEL' FAILED: %s", mosquitto_strerror(retour) ); }
...
/******************************************************************************************************************************/
 static void MQTT_API_on_disconnect_CB( struct mosquitto *mosq, void *obj, int return_code )
  { Info ( __func__, "mqtt", "master", LOG_NOTICE, "Disconnected with return code %d: %s",
              return_code, mosquitto_connack_string( return_code ) );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 gboolean MQTT_Start ( void )
  { gint retour;
    ...
    if (!Global.MQTT_session) { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT session error." ); return(FALSE); }
    ...
    if (Json_get_bool ( Global.config, "mqtt_over_ssl" ) )
      { const gchar *ca_file = Json_get_string ( Global.config, "mqtt_ca_file" );
        ...
        if (! (ca_file || ca_path) )
         { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT TLS setup error: no CA file or CA path found." );
           return(FALSE);
         }

        retour = mosquitto_tls_set( Global.MQTT_session, ca_file, ca_path, NULL, NULL, NULL );
        if ( retour != MOSQ_ERR_SUCCESS )
         { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT TLS setup error: %s", mosquitto_strerror(retour) );
           return(FALSE);
         }

        Info ( __func__, "mqtt", "master", LOG_INFO, "MQTT TLS trust store: cafile='%s', capath='%s'",
        ...
        if ( retour != MOSQ_ERR_SUCCESS )
         { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT TLS options error: %s", mosquitto_strerror(retour) );
           return(FALSE);
         }
      }
    ...
    if ( retour != MOSQ_ERR_SUCCESS )
        { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT Connection to '%s:%d' error: %s", target, port, mosquitto_strerror(retour) );
          return(FALSE);
        }
    Info ( __func__, "mqtt", "master", LOG_INFO, "MQTT starting connection to '%s:%d'.", target, port );

    retour = mosquitto_loop_start( Global.MQTT_session );
    if ( retour != MOSQ_ERR_SUCCESS )
     { Info ( __func__, "mqtt", "master", LOG_ERR, "MQTT loop not started: %s", mosquitto_strerror(retour) );
       return(FALSE);
     }
    Info ( __func__, "mqtt", "master", LOG_INFO, "MQTT Connection to '%s:%d' successfull.", target, port );
    return(TRUE);
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void MQTT_Stop ( void )
  { mosquitto_disconnect( Global.MQTT_session );
    ...
    Info ( __func__, "mqtt", "master", LOG_INFO, "MQTT Stopped" );
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
